(function() {
  alert('Debug: Voice Agent Script is loading');
  
  // Visual Error Logger
  window.addEventListener('error', function(e) {
    const errDiv = document.createElement('div');
    errDiv.style.cssText = 'position:fixed;top:100px;left:10px;background:red;color:white;z-index:2147483647;padding:10px;max-width:300px;word-wrap:break-word;';
    errDiv.innerText = 'JS Error: ' + e.message;
...
Keep responses concise and natural because the interaction is voice-based.`;
    ...
    try {
      ...
      ws.onmessage = async (evt) => {
        ...
        if (evt.data instanceof Blob) {
            // Wait, Gemini API usually sends JSON over text websocket for the beta BidiGenerateContent, or sometimes JSON over blob if the websocket mode is binary? Let's read it as text.
  ...
  function handleFunctionCall(funcCall) {
    ...
    if (funcCall.name === "navigate_scene") {
      ...
      // Fuzzy matching to handle potential model errors (e.g. "back view" instead of "back_view")
